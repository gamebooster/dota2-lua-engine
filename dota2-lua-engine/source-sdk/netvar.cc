...
int NetVarManager::GetOffsetFromTable(
  const char* class_name, const char* var_name, RecvTable* table) {
    int offset = 0;
    ...
    for (int i = 0; i < table->GetNumProps(); i++) {
      ...
      if (!strcmp( table->GetName(), class_name ) &&
          !strcmp(prop->GetName(), var_name)) {
...
int NetVarManager::GetOffsetFromTableL(
  const char* class_name, const char* var_name,
...
int NetVarManager::GetNetVarOffset(
  const char* class_name, const char* var_name) {
    std::string key(class_name);
    key.append(var_name);
    if (cache_[key]) return cache_[key];

    ClientClass* clientClass = GlobalInstanceManager::GetClient()
      ->GetAllClasses();
...
int NetVarManager::GetNetVarOffsetL(
  const char* class_name, const char* var_name, const char* var_name2) {
    std::string key(class_name);
    key.append(var_name);
