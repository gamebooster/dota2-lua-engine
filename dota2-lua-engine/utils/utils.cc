...
std::string utils::GetModuleDirectory() {
  std::string module_path_with_filename =
    utils::GetModulePath(utils::GetCurrentModule());
  char module_drive[kMaxDriveSize];
  char module_dir[kMaxDirSize];

  _splitpath(
    module_path_with_filename.c_str(), module_drive, module_dir, NULL, NULL);
